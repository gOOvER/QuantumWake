...
/// <summary>A part's picture as bytes, with what it is.</summary>
public sealed record PartPicture(byte[] Bytes, string ContentType);

/// <summary>
/// Pictures of ship components, fetched from the Star Citizen Wiki once and
/// kept beside the community digest.
/// </summary>
/// <remarks>
/// <para>
...
/// almost no radars or missile racks); the rest keep the maker's mark.
/// </para>
/// <para>
/// One page-image lookup per part, by its display name, then one download of
/// a thumbnail. Both go to the wiki only after the community dataset was
/// switched on, which is the app's consent to talk to the network, and carry
/// the part's name and nothing else. A miss is remembered for a month so a
/// bench full of radars does not ask the wiki the same question every time
/// it opens.
/// </para>
/// </remarks>
public sealed class PartPictures
{
    ...
    /// <summary>The same part asked for twice while its first fetch is in flight shares that fetch.</summary>
    ...
    {
        _directory = directory ?? AppPaths.In("community", "part-pictures");
        _missesPath = Path.Combine(_directory, "misses.json");
        LoadMisses();
    }
    ...
    public Task<PartPicture?> GetAsync(HttpClient http, PartStats part, CancellationToken token = default)
    {
        ...
        var cached = FromCache(part.Uuid);
        if (cached is not null)
            return Task.FromResult<PartPicture?>(cached);

        lock (_gate)
        {
            if (_misses.TryGetValue(part.Uuid, out var when) && DateTimeOffset.UtcNow - when < MissLifetime)
                return Task.FromResult<PartPicture?>(null);
        }

        return _inFlight.GetOrAdd(part.Uuid, key => FetchAsync(http, part, token)
            .ContinueWith(t =>
            {
    ...
    private async Task<PartPicture?> FetchAsync(HttpClient http, PartStats part, CancellationToken token)
    {
        await _slots.WaitAsync(token);
        try
        {
            var source = await LookUpAsync(http, part.Name, token);
            if (source is null)
            {
                RecordMiss(part.Uuid);
                return null;
            }
        ...
        {
            // Network trouble is not "the wiki has no picture": leave the part
            // askable again next time rather than blanking it for a month.
            return null;
        }
        finally
        {
    ...
    /// <summary>Asks the wiki which picture leads the part's page, trying each title it might be under.</summary>
    private static async Task<string?> LookUpAsync(HttpClient http, string name, CancellationToken token)
    {
        var titles = string.Join('|', Titles(name));
        var url = $"{WikiApi}?action=query&prop=pageimages&piprop=thumbnail&pithumbsize={Width}&redirects=1&format=json&titles={WebUtility.UrlEncode(titles)}";
        ...
        foreach (var title in Titles(name))
    ...
    private PartPicture? FromCache(string uuid)
    {
        foreach (var (ext, type) in Extensions)
        {
            var path = Path.Combine(_directory, uuid + ext);
            if (File.Exists(path))
    ...
    private void Store(string uuid, string type, byte[] bytes)
    {
        var ext = Extensions.FirstOrDefault(e => e.Type == type).Ext ?? ".jpg";
        Directory.CreateDirectory(_directory);
        File.WriteAllBytes(Path.Combine(_directory, uuid + ext), bytes);
    }
    ...
    private void RecordMiss(string uuid)
    {
        lock (_gate)
        {
            _misses[uuid] = DateTimeOffset.UtcNow;
            try
            {
                Directory.CreateDirectory(_directory);
