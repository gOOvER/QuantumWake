...
public sealed partial class GameCommodities
{
    ...
    public static GameCommodities Load(string? installRoot, string cachePath)
    {
        ...
        var (commodities, items, facts, blueprints, spawns, places, wikelo, vehicles, paints, makerLogos, mining) = Read(archive);
        if (commodities.Count > 0 || items.Count > 0)
            SaveCache(cachePath, stamp, commodities, items, facts, blueprints, spawns, places, wikelo, vehicles, paints, makerLogos, mining);

        return new GameCommodities(commodities, items, facts, blueprints, spawns, places, wikelo, vehicles, paints, makerLogos, mining);
    }
    ...
    private static (Dictionary<string, string> Commodities, Dictionary<string, string> Items,
        ...
        List<GameSpawn> Spawns, Dictionary<string, GamePlace> Places, GameWikeloCatalogue Wikelo, Dictionary<string, GameVehicle> Vehicles, List<GamePaint> Paints, Dictionary<string, string> MakerLogos, GameMiningData Mining) Read(string archivePath)
    {
        ...
        try
        {
            ...
            if (blob is null || ini is null)
                return (result, itemUuids, facts, blueprints, spawns, places, wikelo, vehicles, paints, makerLogos, mining);
    ...
    private static void SaveCache(
        ...
        Dictionary<string, GameVehicle> vehicles, List<GamePaint> paints, Dictionary<string, string> makerLogos, GameMiningData mining)
    {
        try
        {
            ...
            File.WriteAllText(cachePath,
                JsonSerializer.Serialize(
                    new Cache
                    {
                        ...
                        Wikelo = wikelo, Vehicles = vehicles, Paints = paints, MakerLogos = makerLogos, Mining = mining
                    },
                    Json));
        }
