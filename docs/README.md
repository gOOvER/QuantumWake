...
| [joystick-mapping.md](joystick-mapping.md) | Research for a Controls page (0.15): where the game keeps joystick bindings and why they break when sticks re-enumerate, the 1,106-action catalogue in CryXmlB, the 17 reference layouts CIG ships, and a probe showing Windows.Gaming.Input reads all six devices here and composes the game's product GUID exactly - what is doable (tables, backups, diff, swap fix, live button press) and what is not (pictures of sticks, editing in place). |
